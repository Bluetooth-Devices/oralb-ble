...
NOT_ORALB_MANUFACTURER = {76}


def decode_temps(packet_value: int) -> float:
    """Decode potential negative temperatures."""
    # https://github.com/Thrilleratplay/OralBWatcher/issues/2
...
class OralBBluetoothDeviceData(BluetoothData):
    ...
    def _start_update(self, service_info: BluetoothServiceInfo) -> None:
        ...
        for mfr_id, mfr_data in manufacturer_data.items():
            if mfr_id in NOT_ORALB_MANUFACTURER:
                continue
    ...
    def _process_mfr_data(
        ...
        _LOGGER.debug("Parsing OralB sensor: %s %s", mgr_id, data)
        msg_length = len(data)
        ...
        if msg_length == 9 and (
            ...
            if sensor_id == 0:
                ...
                self.set_device_manufacturer("OralB", device_id)
            elif sensor_id == 1:
                device_id = "remote"
                self.set_device_name(
                ...
                _LOGGER.debug(
                    "Unknown sensor id for OralB H5178,"
